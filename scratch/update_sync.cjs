...
s = s.replace(
  ...
  `ON CONFLICT(id) DO UPDATE SET front=excluded.front, back=excluded.back,
         ...
         stability=excluded.stability, difficulty=excluded.difficulty, state=excluded.state, lapses=excluded.lapses, last_review=excluded.last_review`
);
s = s.replace(
  ...
  `[c.id, c.setId, userId, c.front, c.back, c.easiness ?? 2.5, c.interval ?? 0,
       c.repetitions ?? 0, c.dueDate ?? null, c.updatedAt, c.deleted ? 1 : 0, now,
       c.stability ?? null, c.difficulty ?? null, c.state ?? null, c.lapses ?? 0, c.lastReview ?? null]`
);
