...
export function createApp(db: DB) {
  ...
  app.get("/v1/insights", async (c) => {
    ...
    const sets = await all<any>(db, "SELECT id, title, exam_date FROM sets WHERE user_id = ? AND (deleted = 0 OR deleted IS NULL)", [userId]);
    const setIds = new Set(sets.map(s => s.id));
    
    const cards = await all<any>(db, "SELECT id, set_id, front, due_date, stability, state, interval, last_review, repetitions, lapses FROM cards WHERE user_id = ? AND (deleted = 0 OR deleted IS NULL)", [userId]);
    
    const revs = await all<any>(db, "SELECT grade, prev_interval FROM review_log WHERE user_id = ? AND kind = ? AND reviewed_at >= ? AND prev_interval >= 1 LIMIT 5000", [userId, "flashcard", thirtyDaysAgo]);
