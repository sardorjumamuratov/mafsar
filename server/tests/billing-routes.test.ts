...
vi.mock("stripe", () => ({
  default: class StripeMock {
    ...
    subscriptions = { retrieve: vi.fn(async (id: string) => ({ id, status: "active" })) };
...
describe("stripe webhook", () => {
  ...
  it("subscription.updated active + cancel_at_period_end does NOT downgrade", async () => {
    ...
    const res = await sendEvent({
      ...
      data: { object: { customer: "cus_123", status: "active", cancel_at_period_end: true } },
    });
    expect(res.status).toBe(200);
    const row = await one<{ plan: string }>(db, "SELECT plan FROM users WHERE id = ?", [user.id]);
...
describe("checkout + portal routes", () => {
  ...
  it("checkout creates a session and returns its url", async () => {
