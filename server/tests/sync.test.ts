...
import { openDB, uid, nowISO, type DB } from "../src/db.js";
...
describe("sync", () => {
  ...
  it("merges activity per-day with max", async () => {
    const id = await userId();
    applySync(db, id, baseBody({ activity: [{ day: "2026-08-14", count: 3 }] }));
    applySync(db, id, baseBody({ activity: [{ day: "2026-08-14", count: 7 }] }));
    applySync(db, id, baseBody({ activity: [{ day: "2026-08-14", count: 5 }] }));
    expect(changesSince(db, id).activity[0].count).toBe(7);
  });
  ...
  it("is idempotent — replaying the same batch changes nothing", async () => {
    ...
    applySync(db, id, batch);
    applySync(db, id, batch);
    expect(changesSince(db, id).reviews).toHaveLength(1);
  });
